...
Код управляет шаговыми моторами с помощью FreeRTOS, тестировался на сборке со светодиодами, на манипуляторе не запускался

Функция управляющая шаговыми моторами самописная, реализованы ускорения, но не выставлена максимальная скорость, не прописаны критические углы

Принимает значения через COM порт в формате changeMotorTask(0/1) angle speed acceleration
