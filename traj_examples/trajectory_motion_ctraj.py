import matplotlib.pyplot as plt
import roboticstoolbox as rtb
import spatialmath as sm
import spatialgeometry as sg
import swift
import numpy as np
import time

env = swift.Swift()
env.launch(realtime=True)

robot = rtb.models.MEPhI_ARM()
robot.q = robot.qr
env.add(robot)

time.sleep(1)

Tep1 = robot.fkine(robot.q) * sm.SE3.Trans(0, 0, 0.2) * sm.SE3.OA([0, 1, 0], [0, 0, 1])
sol1 = robot.ik_LM(Tep1)
goal_ax = sg.Axes(0.1, pose=Tep1)
env.add(goal_ax)
traj1 = rtb.tools.trajectory.jtraj(robot.qr, sol1[0], 100)
# print(traj1.q)
for i in traj1.q:  # for each joint configuration on trajectory
    robot.q = i  # update the robot state
    env.step(0.01)  # update visualization

Tep2 = robot.fkine(robot.q) * sm.SE3.Trans(0.2, 0, 0) * sm.SE3.OA([0, 1, 0], [0, 0, 1])  # Задание целевой позы
goal_ax = sg.Axes(0.1, pose=Tep2)  # построение системы координат в ней
env.add(goal_ax)

travel_time = 3
step = 50
step_time = travel_time / step
time_vec = np.arange(0, travel_time, step_time)

traj2 = rtb.tools.trajectory.ctraj(Tep1, Tep2, time_vec)  # построение траектории
cartesian_sol = robot.ikine_LM(traj2)  # получение обобщенных координат каждой точки траектории
# print(cartesian_sol.q)

velocity_array = np.empty((step - 1, 6))  # Создание массива для хранения скоростей в осях
for i in range(step - 1):  # вычисление обобщенных скоростей в осях для каждой точки траектории
    for k in range(6):
        velocity_array[i][k] = (cartesian_sol.q[i + 1][k] - cartesian_sol.q[i][k]) / step_time
# np.set_printoptions(linewidth=300, suppress=True)
# print(velocity_array)

graphics = velocity_array.transpose()  # Отображение графиков скоростей в осях
for i in range(6):
    if i != 6:
        plt.plot(graphics[i], label=f"{i + 1}")
plt.legend()
plt.show()

for i in cartesian_sol.q:  # Отображение позы перебором обобщенных координат
    # goal_ax = sg.Axes(0.1, pose=robot.fkine(i)) # Построение траектории
    # env.add(goal_ax)
    robot.q = i
    env.step(step_time)

# for z in range(step - 1):  # Отображение позы заданием скоростей
#     robot.qd = velocity_array[z]
#     # link_ax = sg.Axes(0.1, pose=robot.fkine(robot.q))  # Построение траектории
#     # env.add(link_ax)
#     env.step(step_time)
