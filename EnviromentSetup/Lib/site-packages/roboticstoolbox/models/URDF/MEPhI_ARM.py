...
class MEPhI_ARM(Robot):
    ...
    def __init__(self):
        links, name, urdf_string, urdf_filepath = self.URDF_read(
            #"puma560_description/urdf/MEPhI_ARM_gripper.urdf"
            "MEPhI_ARM_description/urdf/MEPhI_ARM_gripper.urdf"
        )

        super().__init__(
            links,
            name=name,
            manufacturer="Engi-Team",
            gripper_links=links[7],
            urdf_string=urdf_string,
            urdf_filepath=urdf_filepath,
        )

        self.qr = np.array([0, 0, 0, 0, pi/2, 0])
        self.qz = np.zeros(6)
